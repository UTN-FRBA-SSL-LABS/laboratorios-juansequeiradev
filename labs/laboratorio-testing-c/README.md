...
> R:Inicializa el carrito con cantidad = 0 y sin productos para que no tenga valores basura
...
> R:El bug esta en la linea 24 y el problema es que el total esta calculandose con el precio de cada producto en sumatoria pero sin contemplar la cantidad del producto

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Simplemente se debe multiplicar por la cantidad el precio de cada producto
...
> R:Falla ya que el ultimo carrito agregar devuelve uno y permite agregar el 5to elemento
...
> R:El operador incorrecto es <= cuando deberia ser solo < ya que cantidad en carrito empieza en 0 debe ser menor a 4 para que al llegar a 3 no permita agregar un 5to elemento
...
> R:Las lineas 29 y 30 que abarcan la funcion carrito_descuento no fueron ejecutadas por los test unitarios por lo cual no estan cubiertas
...
> R:Un test unitario prueba la funcionalidad correcta de las funciones una por una mientras que el de integracion las prueba en su conjunto. El unitario detecta primero el bug de carrito total.
...
> R:Esto es peligroso en C porque el lenguaje no mide o verifica los limites, al excederse sobreescribe memoria adyacente, en Java o Python el programa falla pero de forma mas segura sin chance de corromper memoria del sistema.
...
> R:Mirar el codigo de manera directa en busca de errores es probar casos en tu cabeza, quieras o no en algun momento te podes confundir, olvidar o ignorar algun caso de uso que no contemplaste antes o pensabas que tuviste en cuenta, haciendo los tests en escrito y codigo es mucho mas estricto y verificable que todo ande bien.
...
> R:Porque ambos estan planteados para casos de uso distintos, como debe de ser. Si intentaramos probar una funcion con varias formas de utilizarse en un solo test, no solo seria larguisimo si no que tambien seria desorganizado y poco practico, para eso planteamos otro caso de uso o test donde verificamos correctamente que habia una falla al usar la funcion de cierta manera.
